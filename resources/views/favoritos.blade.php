        ...
        <main class="flex-1 lg:pl-72 p-6 space-y-8">
            <div class="container mx-auto p-4">
                ...
                <header class="text-center mb-8">
                    <h1 class="text-4xl font-bold text-gray-800">Mis Libros Favoritos</h1>
                </header>

                <form method="GET" action="{{ route('favoritos') }}" class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        @foreach ($favoritos as $libro)
                            <div
                                ...
                                <div class="flex justify-end items-center p-4 border-t border-gray-100 space-x-4">
                                    ...
                                    <form method="POST" action="{{ route('delFav', $libro->id) }}" onsubmit="return confirm('¿Estás seguro que quieres eliminar este libro?');">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <form method="POST" action="{{ route('quitFav', $libro->id) }}" onsubmit="return confirm('¿Quieres quitar este libro de tus deseados?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @else
                        <div class="bg-gray-100 rounded-lg shadow-md p-6 flex items-center space-x-6">
                            <h1>Aún no agregas Libros Favoritos</h1>
                        </div>
                    @endif
                </div>
